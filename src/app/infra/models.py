...
class ORMUser(Base):
    ...
    transactions = relationship("ORMTransaction", back_populates="owner")
...
class ORMTransaction(Base):
    ...
    created_at = Column(DateTime, default=datetime.now(UTC))
    owner_id = Column(Integer, ForeignKey("users.id"))
    owner = relationship("ORMUser", back_populates="transactions")
...
class ORMPredictionJob(Base):
    ...
    user = relationship("ORMUser", backref="prediction_jobs")
