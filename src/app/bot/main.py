...
_user_clients: dict[int, ApiClient] = {}
def api_for(tgid: int) -> ApiClient:
    return _user_clients.setdefault(tgid, ApiClient())


async def safe_call(msg: types.Message, coro):
    """
    Выполняет REST-запрос и выводит ошибку.
    Возвращает результат или None (если была ошибка).
    """
    try:
        return await coro
...
async def register(msg: types.Message):
    ...
    if await safe_call(msg, api_for(msg.from_user.id).register(email, pwd)) is None:
        return
...
async def login(msg: types.Message):
    try:
        ...
        await msg.answer("Формат: `/login email pass`"); return
    if await safe_call(msg, api_for(msg.from_user.id).login(email, pwd)) is not None:
...
async def topup(msg: types.Message):
    ...
    if len(parts) != 2 or not parts[1].isdigit():
        await msg.answer("Формат: `/topup 100`"); return
