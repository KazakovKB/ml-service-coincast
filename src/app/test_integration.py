...
def transaction_record(user: Client):
...
def test_user_creation_and_account(db):
    user = Client(email="testuser@example.com", password_hash=Client.hash_password("pass"))
...
def test_deposit_by_user(db):
    user = Client(email="user@example.com", password_hash=Client.hash_password("pass"))
    orm_user = create_user(user)
    db.add(orm_user)
    db.commit()
    db.refresh(orm_user)
    ...
    orm_tx = transaction_record(user)
    db.add(orm_tx)
    db.commit()
    db.refresh(orm_account)

    assert orm_account.balance == 200
    txs = db.query(ORMTransaction).filter_by(owner_id=orm_user.id).all()
    assert len(txs) == 1
    assert txs[0].amount == 200
    assert txs[0].tx_type == TxType.DEPOSIT

def test_deposit_by_admin(db):
    admin = Admin(email="admin@example.com", password_hash=Admin.hash_password("admin123"))
    user = Client(email="user@example.com", password_hash=Client.hash_password("user123"))
...
def test_transaction_history(db):
    ...
    orm_account.balance = user.account.balance
    db.add(transaction_record(user))
    db.commit()
    db.refresh(orm_account)
    # Списание
